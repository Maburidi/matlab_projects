...
f_gp= y_gp1_true;          % k_gp positions of SV are randomly distributed
f_p = y_p1_true;                 % k_p positions of SV are randomly distributed
f_c = y_c_true ;                 % k_c positions of SV are randomly distributed

common= length(find(f_p(find(f_gp == 1)) ==1))
common= length(find(f_c(find(f_gp == 1)) ==1))
...
%Use if looping tau
        ...
        [fhatSPIRAL_1p1c_nov, iterationsSPIRAL, objectiveSPIRAL,...
            reconerrorSPIRAL, cputimeSPIRAL] ...
            = SPIRALTAP_1GP1P1C_Hap(y,A,tau,...
            'maxiter',maxiter,...
        ...
        [fhatSPIRAL_1p1c_nov_2, iterationsSPIRAL_2, objectiveSPIRAL_2,...
            reconerrorSPIRAL_2, cputimeSPIRAL_2] ...
            = SPIRALTAP_1GP1P1C_Hap_II(y,A,tau,...
            'maxiter',maxiter,...
            'Initialization',finit,...
            'AT',AT,...
            'miniter',5,...
  ...
  fhatSPIRAL_c_2d= fhatSPIRAL_1p1c_nov_2(1:n);
  fhatSPIRAL_p_2d = fhatSPIRAL_1p1c_nov_2(n+1:2*n);
  fhatSPIRAL_gp_2d = fhatSPIRAL_1p1c_nov_2(2*n+1:3*n);

% separate reconstruction for each true signal   3d 
  fhatSPIRAL_c = fhatSPIRAL_1p1c_nov(1:n);
  fhatSPIRAL_p = fhatSPIRAL_1p1c_nov(n+1:2*n);
  fhatSPIRAL_gp = fhatSPIRAL_1p1c_nov(2*n+1:3*n);
...
% ROC curve of all signal 
figure(1)                     
[X1,Y1,T1,AUC1] = perfcurve(f_true,f_spiral,'1');     % 3d
[X2,Y2,T2,AUC2] = perfcurve(f_true,f_spiral_2d,'1');   % 2d 
...
dim1 = [.2 .5 .3 .1];
dim2 = [.2 .5 .3 .01];
str1 = strcat('AUC_{3D}= ',num2str(Area_recall_3d));
str2 = strcat('AUC_{2D}= ',num2str(Area_recall_2d));
annotation('textbox',dim1,'String',str1,'FitBoxToText','on');
annotation('textbox',dim2,'String',str2,'FitBoxToText','on');
legend("3D Projections","2D Projections","Location","best")
title('All Signals')
...
% ROC curve of all signal 
figure(3)                     
[X1,Y1,T1,AUC1] = perfcurve(f_c_true,f_c_spiral,'1');     % 3d
[X2,Y2,T2,AUC2] = perfcurve(f_c_true,f_c_spiral_2d,'1');   % 2d 
AreaUnerCurve_3d =1+trapz(1-Y1,X1);
AreaUnerCurve_2d =1+trapz(1-Y2,X2);
plot(X1,Y1,'blue','LineWidth',3) ;hold on;
plot(X2,Y2,'red','LineWidth',3) ;hold on;
...
% ROC curve of all signal 
figure(5)                     
[X1,Y1,T1,AUC1] = perfcurve(f_p_true,f_p_spiral,'1');     % 3d
[X2,Y2,T2,AUC2] = perfcurve(f_p_true,f_p_spiral_2d,'1');   % 2d 
AreaUnerCurve_3d =1+trapz(1-Y1,X1);
AreaUnerCurve_2d =1+trapz(1-Y2,X2);
plot(X1,Y1,'blue','LineWidth',3) ;hold on;
plot(X2,Y2,'red','LineWidth',3) ;hold on;
...
% ROC curve of all signal 
figure(7)                     
[X1,Y1,T1,AUC1] = perfcurve(f_gp_true,f_gp_spiral,'1');     % 3d
[X2,Y2,T2,AUC2] = perfcurve(f_gp_true,f_gp_spiral_2d,'1');   % 2d 
AreaUnerCurve_3d =1+trapz(1-Y1,X1);
AreaUnerCurve_2d =1+trapz(1-Y2,X2);
plot(X1,Y1,'blue','LineWidth',3) ;hold on;
plot(X2,Y2,'red','LineWidth',3) ;hold on;
...
%%%%%%%%%%%%%%%%%%%%%%%%%% SPIRAL SECTION %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [x, varargout] = SPIRALTAP_1GP1P1C_Hap(y, A, tau, varargin)
        ...
        case 1 % Barzilai-Borwein choice of alpha
            if monotone 
                ...
                while (accept == 0)
                    ...
                    dx = xprevious;
                    step = xprevious - grad./alpha;
                    x = computesubsolution(step,tau,alpha,penalty,mu,iter,...
                        W,WT,subminiter,submaxiter,substopcriterion,...
                        subtolerance);
                    dx = x - dx;
                    Adx = Axprevious;
...
function [x, varargout] = SPIRALTAP_1GP1P1C_Hap_II(y, A, tau, varargin)
% ==== Set default/initial parameter values ====
% ---- All Methods ----
% =============================================================================
% =        SPIRAL:  Sparse Poisson Intensity Reconstruction Algorithms        =
        ...
        case 1 % Barzilai-Borwein choice of alpha
            if monotone 
                ...
                while (accept == 0)
                    ...
                    dx = xprevious;
                    step = xprevious - grad./alpha;
                    x = computesubsolution_II(step,tau,alpha,penalty,mu,iter,...
                        W,WT,subminiter,submaxiter,substopcriterion,...
                        subtolerance);
                    dx = x - dx;
                    Adx = Axprevious;
...
% =====================================
function subsolution = computesubsolution(step,tau,alpha,penalty,mu,iter,varargin)
    switch lower(penalty)
        case 'canonical'
...
function subsolution = computesubsolution_II(step,tau,alpha,penalty,mu,iter,varargin)
    switch lower(penalty)
        case 'canonical'
            ...
            %if proj_2d == 1   %2D projections 
             [subsolution] = Projections_2D_Alternating_II(subsolution,10);      
            %else % do 3D projections 
             %[subsolution] = Projections_3D(subsolution);                   
            %end 
...
function [ f_subs ] = Projections_2D_Alternating(f_vec,maxit)

% The following function enforces constraints of problem within feasible
% region for the minimization subproblem of the form:
% 
%  phi ( f_c, f_p1, f_p2 ) = 1/2*((f_c - c)^2 + (f_p1 - p1)^2 + (f_p2 -
%  p2)^2)
%
% where c = s_c - lambda, p1 = s_p1 - lambda, p2 = s_p2 - lambda
...
while (j < maxit)
    ...
    [c_new, p_new] = feasfix_oneindv(c, p, gp);
    
    % Update c and p, then fix them to project onto 
    c = c_new;
    p = p_new; 

    [c_new, gp_new] = feasfix_oneindv(c, gp, 1);

    % Update c and p, then fix them to project onto 
    c = c_new;
    gp = gp_new; 

    [p_new, gp_new] = feasfix_oneindv(p, gp, 1);
...
function [ f_subs ] = Projections_2D_Alternating_II(f_vec,maxit)
...
function [a_new,b_new] = feasfix_oneindv(a,b,c) 
