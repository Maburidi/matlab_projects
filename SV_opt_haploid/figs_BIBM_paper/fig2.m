...
close all 
clear dis_2d 
origion_ = zeros(1,length(f_true)) ;

for j=1:iterationsSPIRAL_2
dis_2d(j) = pdist2(origion_,solutionpathSPIRAL_2d(j).iterate'); 
end 

for i=1:iterationsSPIRAL
dis_3d(i) = pdist2(origion_,solutionpathSPIRAL_3d(i).iterate');
end 

thresh_ = 0.478;
f_2d = solutionpathSPIRAL_2d(iterationsSPIRAL_2).iterate >= thresh_;
f_3d = solutionpathSPIRAL_3d(iterationsSPIRAL).iterate >= thresh_;

loglog(1:iterationsSPIRAL+4,dis_2d(1:iterationsSPIRAL+4),"Color","red","LineWidth",2); hold on
loglog(1:iterationsSPIRAL,dis_3d,"Color","blue","LineWidth",2);
xlabel("Iteration Number","FontSize",20)
ylabel("Distance from Origion","FontSize",20)
dis_2d_3d = pdist2(f_2d' , f_3d');  

dim1 = [.3 .5 .3 .1];

str1 = strcat('Distance between both solutions = ',num2str((dis_2d_3d)));
annotation('textbox',dim1,'String',str1,'FitBoxToText','on');

legend("2D Solution", "3D Solution")
title("Solution Path","FontSize",20)
xlim([0 73])
